 ...
 * Ставки (рыночные ориентиры в BYN) и partnerDiscount — скидка «для своих» — хранятся в apps-script/Rates.local.gs,
 * он не попадает в публичный репозиторий; здесь rate: 0 — заглушки.
 */
var KP = {
  dev: {
    name: 'Елена Саманчук',
  ...
  modules: {
    taplink: { title: 'Мини-сайт для соцсетей: кнопки, контакты, главное о вас', rate: 0, days: 3 },
    landing: { title: 'Одностраничный сайт: дизайн, адаптация под телефоны, форма заявки', rate: 0, days: 7 },
    landingBrand: { title: 'Ещё один бренд на странице — свой блок и характер', rate: 0, days: 2 },
    multi: { title: 'Сайт из нескольких страниц: дизайн, адаптация под телефоны, до 6 страниц', rate: 0, days: 14 },
    extraPage: { title: 'Дополнительные страницы', rate: 0, days: 1 },
    ...
    linksPage: { title: 'Страница-мультиссылка для соцсетей', rate: 0, days: 1 },
    catalogPage: { title: 'Товары, услуги или меню с фото и ценами', rate: 0, days: 3 },
    teamPage: { title: 'Команда: мастера, специалисты или преподаватели', rate: 0, days: 1 },
    galleryPage: { title: 'Фото и работы: галерея или кейсы', rate: 0, days: 1 },
    schedule: { title: 'Расписание и старт групп', rate: 0, days: 2 },
    cart: { title: 'Корзина и оформление заказа — заказ приходит вам в Telegram', rate: 0, days: 6 },
    payment: { title: 'Онлайн-оплата картой и через ЕРИП, статусы оплаты', rate: 0, days: 5 },
    shopDocs: { title: 'Оферта, условия доставки и возврата', rate: 0, days: 1 },
    banquets: { title: 'Мероприятия и банкеты: залы, меню, форма заявки', rate: 0, days: 2 },
    bookingForm: { title: 'Заявка на запись или бронь', rate: 0, days: 1 },
    bookingService: { title: 'Подключение вашего сервиса записи', rate: 0, days: 1 },
    booking: { title: 'Онлайн-запись с выбором даты и времени', rate: 0, days: 4 },
    map: { title: 'Карта точек: «открыто сейчас», маршрут, ближайшая точка', rate: 0, days: 2 },
    daily: { title: 'Предложение или меню дня, которое вы обновляете сами', rate: 0, days: 2 },
    gsheet: { title: 'Цены из Google-таблицы — правите таблицу, сайт обновляется', rate: 0, days: 3 },
    pos_sync: { title: 'Выгрузка меню или товаров из учётной программы', rate: null, days: 5 },
    delivery_zones: { title: 'Зоны доставки на карте с разной ценой', rate: 0, days: 3 },
    b2b: { title: 'Раздел для организаций: обеды в офисы, опт, договоры', rate: 0, days: 2 },
    remind: { title: 'Напоминания клиентам о записи', rate: 0, days: 2 },
    lms: { title: 'Онлайн-обучение: доступ к урокам и заданиям', rate: null, days: 10 },
    compare: { title: 'Избранное и сравнение товаров', rate: 0, days: 3 },
    reviews_collect: { title: 'Сбор отзывов после визита или покупки', rate: 0, days: 2 },
    calc: { title: 'Калькулятор стоимости', rate: 0, days: 3 },
    banqcalc: { title: 'Калькулятор стоимости банкета', rate: 0, days: 3 },
    quiz: { title: 'Квиз: пара вопросов → расчёт или подборка → заявка', rate: 0, days: 3 },
    reviews: { title: 'Рейтинг и отзывы с Яндекс Карт', rate: 0, days: 1 },
    promo: { title: 'Акции с таймером и баннеры, которые включаете сами', rate: 0, days: 2 },
    game: { title: 'Игровая механика: колесо фортуны или карточки со скидками', rate: 0, days: 3 },
    blog: { title: 'Блог или раздел статей', rate: 0, days: 3 },
    events: { title: 'Афиша событий', rate: 0, days: 2 },
    jobs: { title: 'Вакансии с откликом и загрузкой резюме', rate: 0, days: 2 },
    certs: { title: 'Подарочные сертификаты', rate: 0, days: 2 },
    ...
    orderPromo: { title: 'Промокоды', rate: 0, days: 1 },
    account: { title: 'Личный кабинет клиента', rate: 0, days: 5 },
    orderStatus: { title: 'Статус заказа для клиента', rate: 0, days: 3 },
    orderRepeat: { title: 'Кнопка «повторить заказ»', rate: 0, days: 1 },
    lang: { title: 'Версия на другом языке', rate: 0, days: 3 },
    copy: { title: 'Тексты для сайта', rate: 0, days: 3 },
    copyEdit: { title: 'Редактура и доработка ваших текстов', rate: 0, days: 2 },
    styleKit: { title: 'Лёгкий фирменный стиль: цвета, шрифты, графические элементы', rate: 0, days: 3 },
    vectorize: { title: 'Перерисовка логотипа в вектор', rate: 0, days: 1 },
    seo: { title: 'Базовое SEO: заголовки, описания, карта сайта, разметка, Яндекс Вебмастер', rate: 0, days: 2 },
    analytics: { title: 'Яндекс Метрика и цели на заявки', rate: 0, days: 1 },
    privacy: { title: 'Политика обработки персональных данных и согласия в формах', rate: 0, days: 1 }
  },
  extras: {
...
// Ставки и скидка «для своих» — в apps-script/Rates.local.gs (в git не попадает, в бандл добавляется при сборке)
if (typeof KP_RATES_LOCAL !== 'undefined') Object.keys(KP_RATES_LOCAL).forEach(function (k) { if (KP.modules[k]) KP.modules[k].rate = KP_RATES_LOCAL[k]; });
if (typeof KP_DISCOUNT_LOCAL !== 'undefined') KP.partnerDiscount = KP_DISCOUNT_LOCAL;
